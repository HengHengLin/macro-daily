...
OUT_PATH = os.path.expanduser("~/macro_daily/daily_data.json")
...
def fred_latest(series_id):
    try:
        resp = requests.get(
            ...
            params={"series_id": series_id, "api_key": "DEMO",
                    "file_type": "json", "sort_order": "desc", "limit": 2},
            timeout=12)
        obs = resp.json()["observations"]
        cur = {"value": obs[0]["value"], "date": obs[0]["date"]}
        if len(obs) > 1:
            try: cur["change"] = round(float(obs[0]["value"]) - float(obs[1]["value"]), 4)
            except: pass
        return cur
    except Exception as e:
        return {"value": "N/A", "date": "N/A", "error": str(e)[:60]}

def fetch_macro_us():
    return {
        "fed_balance_sheet": fred_latest("WALCL"),
        "m2":                fred_latest("M2SL"),
        "cpi":               fred_latest("CPIAUCSL"),
        "core_cpi":          fred_latest("CPILFESL"),
        "pce":               fred_latest("PCEPI"),
        "ppi":               fred_latest("PPIACO"),
        "unemployment":      fred_latest("UNRATE"),
        "ism_pmi":           fred_latest("NAPM"),
    }
...
def fetch_macro_cn():
    ...
    cn["social_finance"] = safe_ak(
        ...
        lambda df: {"date":     str(df.iloc[0].get("月份","N/A")),
                    "loan_yoy": str(df.iloc[0].get("人民币贷款-同比增长","N/A"))},
        "社融")
